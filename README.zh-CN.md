...
通用 Qwen3-8B -> Qwen3-14B low-rank bridge artifact 仍然**没有**通过严格质量门，因此会正确
fallback。现在新增的 prefix-specific 校准 artifact 已经在记录的校准前缀上通过严格 offline
和 runtime 质量门：
...
这是有作用域的校准 proof，不是通用 8B -> 14B bridge。
