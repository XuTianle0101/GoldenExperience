...
| GoldenScale | 同一模型的不同参数规模 | 在 8B <-> 14B 等变体之间复用 KV | 形状匹配时直接别名；否则逐层投影 | 层/头映射与投影校准 |
...
本仓库目前包含 GoldenExperience planner、元数据模型、补丁清单和部署封装。真正的
LMCache hook 实现是下一步。在 `lmcache_cross_model_lookup` 和
`goldenexperience_materializer` 被接入 LMCache 补丁或 fork 之前，
vLLM + LMCache MP + Mooncake Store 会正常运行，GoldenExperience 可以验证计划/元数据，
但被接受的跨模型 KV 复用还不会在 LMCache MP 内部执行。

## GoldenScale 复用

首个 GoldenScale MVP 面向 `Qwen/Qwen3-8B` 与 `Qwen/Qwen3-14B` 的双向复用。GoldenExperience 会把每个方向视为独立 artifact，因为 8B->14B 和 14B->8B 需要不同的层映射、投影规格、成本估计和质量门控。
...
- 任何 tokenizer、RoPE、配置哈希、artifact、层映射、投影或质量不匹配，都会回退到原始的 vLLM + LMCache MP 目标 prefill 路径。
