...
- `scripts/run_cross_model_hidden_bridge_runtime.py`：带质量门的 hidden-bridge 路径。它先做
  source offload，在目标 miss 后执行 `lmcache_cross_model_lookup` 查源模型 Mooncake keys，
  调用 `goldenexperience_materializer`，把 target-shaped chunks 写入 Mooncake 和持久
  `GE_MOONCAKE_EXTERNAL_INDEX`，只有质量门通过时才让新的目标 vLLM 消费注入的 keys。

通用 Qwen3-8B -> Qwen3-14B low-rank bridge artifact 仍然**没有**通过质量门，因此会正确
fallback。历史 prefix-specific artifact
`artifacts/cross_model_runtime/manifests/prefix_specific_strict_20260709T0253Z.json`
只能证明 retrieval：它没有隔离的 target-native phase，且记录的任务断言为 false，因此不满足
当前 strict 语义成功门。
它与同模型 Qwen3-14B offload -> restart -> reuse baseline 的对比是
...
它只是历史 retrieval 证据，不是通用 8B -> 14B bridge。
