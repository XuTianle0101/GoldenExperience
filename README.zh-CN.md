...
hidden-bridge LMCache hook 实现仍是下一步。在 `lmcache_cross_model_lookup` 和
hidden-state `goldenexperience_materializer` 被接入 LMCache 补丁或 fork 之前，
GoldenExperience 还不会把 learned hidden bridge 产物直接注入 LMCache MP。

当前已有一个 runtime proof 路径：`scripts/run_cross_model_runtime.py` 会在同一个
vLLM + LMCache MP + Mooncake Store runtime 中顺序运行源模型和目标模型，使用
`native_target_seed` 生成目标形状 KV，然后重启目标 vLLM 并验证外部 KV 命中。该路径用于证明
runtime 管线和 target-shaped KV retrieval 已接通，不等同于 hidden-bridge 跨参数量注入已经通过质量门。
